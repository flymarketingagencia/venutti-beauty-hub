...
function CheckoutPage() {
  ...
  return (
    <div className="min-h-screen bg-checkout-bg pb-20 text-foreground lg:pb-0">
      ...
      {mobileSummary && <div className="border-b border-checkout-line bg-card px-4 py-4 lg:hidden"><OrderSummary {...{ items, total, subtotal, shippingPrice, shipping, coupon, setCoupon, couponState, applyCoupon, couponDiscount, pixDiscount, payment, updateItem, toggleOffer }} /></div>}

      <main className="mx-auto grid w-full max-w-[1240px] gap-8 px-4 py-5 sm:px-6 sm:py-7 lg:grid-cols-[minmax(0,1.42fr)_minmax(360px,0.88fr)] lg:items-start lg:px-8 lg:py-12">
        <h1 className="sr-only">Finalizar compra</h1>
        <div className="min-w-0 space-y-4 sm:space-y-5">
          <CheckoutSection number="1" title="Seus dados" subtitle="Não precisa criar conta. Leva menos de 3 minutos." open complete={step > 1}>
            ...
            <label className="mt-4 flex min-h-10 cursor-pointer items-start gap-3 text-sm leading-relaxed text-muted-foreground sm:mt-5">
          ...
          <CheckoutSection number="2" title="Entrega" open complete={step > 2}>
          ...
          <CheckoutSection number="3" title="Pagamento" open>
            ...
            <FinalButton total={total} payment={payment} processing={processing} onClick={finish} />
          </CheckoutSection>
        ...
        <aside className="hidden min-w-0 space-y-5 lg:block">
          <div className="sticky top-6 space-y-5">
            <div className="rounded-2xl border border-checkout-line bg-card p-6 shadow-checkout"><OrderSummary {...{ items, total, subtotal, shippingPrice, shipping, coupon, setCoupon, couponState, applyCoupon, couponDiscount, pixDiscount, payment, updateItem, toggleOffer }} /></div>
            <Guarantees />
            <Reviews />
          </div>
        </aside>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-t border-checkout-line bg-card px-4 py-2.5 shadow-[0_-8px_24px_var(--checkout-shadow)] lg:hidden"><div className="min-w-0"><span className="block text-[10px] text-muted-foreground">{payment === "pix" ? "Total no Pix · 5% OFF" : "Total"}</span><strong className={`block truncate text-lg ${payment === "pix" ? "text-success" : ""}`}>{brl(total)}</strong></div><Button onClick={finish} disabled={processing} className="h-12 rounded-xl bg-accent px-4 text-accent-foreground hover:bg-accent/90">{processing ? <LoaderCircle className="animate-spin" /> : <LockKeyhole />} {payment === "pix" ? `Pagar no Pix · ${brl(total)}` : `Finalizar · ${brl(total)}`}</Button></div>
    </div>
  );
}
...
function CheckoutHeader({ step }: { step: number }) {
  ...
  return <header className="border-b border-checkout-line bg-card"><div className="mx-auto max-w-[1240px] px-4 py-4 sm:px-6 sm:py-5 lg:px-8"><div className="grid grid-cols-[1fr_auto_1fr] items-center"><img src={logoAsset.url} alt="Venutti Cosméticos" className="col-start-2 h-10 w-36 object-contain sm:h-11 lg:col-start-1 lg:justify-self-start" /><div className="col-start-3 flex items-center justify-end gap-2 text-xs font-medium text-muted-foreground"><LockKeyhole className="h-4 w-4 text-accent" /><span className="hidden sm:inline">Compra 100% segura</span></div></div><ol className="mx-auto mt-4 grid max-w-2xl grid-cols-4 gap-1 sm:mt-5">{stages.map((stage, index) => { const number = index; const active = (step === 1 ? 1 : step === 2 ? 2 : 3) === number; const complete = number < (step === 1 ? 1 : step === 2 ? 2 : 3); return <li key={stage} className="relative flex flex-col items-center gap-1 text-center sm:gap-1.5"><div className={`absolute left-0 right-0 top-3 h-px ${index === 0 ? "left-1/2" : index === 3 ? "right-1/2" : ""} bg-checkout-line`} /><span className={`relative z-10 grid h-7 w-7 place-items-center rounded-full border text-[11px] font-semibold ${active ? "border-accent bg-accent text-accent-foreground" : complete ? "border-success bg-success text-success-foreground" : "border-checkout-line bg-card text-muted-foreground"}`}>{complete ? <Check className="h-3.5 w-3.5" /> : index + 1}</span><span className={`text-[10px] sm:text-xs ${active ? "font-semibold text-accent" : "text-muted-foreground"}`}>{stage}</span></li>; })}</ol></div></header>;
}
...
function OrderBumps({ items, onToggle }: { items: CartItem[]; onToggle: (offer: (typeof offers)[number]) => void }) {
  return <section className="mt-4 border-b border-checkout-line bg-checkout-offer px-3 py-4 sm:mt-5 sm:rounded-xl sm:border"><div className="flex items-center gap-2"><TicketCheck className="h-5 w-5 text-accent" /><h3 className="font-sans text-sm font-semibold">Ofertas especiais para seu pedido</h3></div><div className="mt-3 grid gap-2">{offers.map((offer) => { const added = items.some((item) => item.id === offer.id); const discount = Math.round((1 - offer.price / offer.oldPrice) * 100); return <article key={offer.id} className={`rounded-xl border bg-card p-3 transition ${added ? "border-accent shadow-sm" : "border-checkout-line"}`}><div className="grid grid-cols-[56px_minmax(0,1fr)] gap-3"><img src={offer.image} alt={offer.name} className="aspect-square w-14 rounded-lg bg-checkout-soft object-cover" /><div className="min-w-0"><h4 className="text-xs font-semibold leading-tight">{offer.name}</h4><p className="mt-1 text-[10px] leading-snug text-muted-foreground">{offer.benefit}</p><div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-[11px]"><span className="text-muted-foreground line-through">{brl(offer.oldPrice)}</span><strong>{brl(offer.price)}</strong><span className="rounded-full bg-success-soft px-1.5 py-0.5 font-bold text-success">-{discount}%</span></div></div></div><Button type="button" variant={added ? "outline" : "default"} onClick={() => onToggle(offer)} className={`mt-2.5 h-9 w-full rounded-lg text-xs ${added ? "border-accent text-accent" : "bg-accent text-accent-foreground hover:bg-accent/90"}`}>{added ? <><Check /> Adicionado · Remover</> : <><Plus /> Adicionar ao pedido</>}</Button></article>; })}</div></section>;
}

function FinalButton({ total, payment, processing, onClick }: { total: number; payment: Payment; processing: boolean; onClick: () => void }) {
  return <div className="mt-6 hidden lg:block"><Button onClick={onClick} disabled={processing} className="h-14 w-full rounded-xl bg-accent text-base text-accent-foreground hover:bg-accent/90">{processing ? <><LoaderCircle className="animate-spin" /> Processando…</> : <><LockKeyhole /> {payment === "pix" ? `Pagar no Pix · ${brl(total)}` : `Finalizar compra · ${brl(total)}`}</>}</Button><div className="mt-3 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-[11px] text-muted-foreground"><span className="flex items-center gap-1"><LockKeyhole className="h-3.5 w-3.5" /> Pagamento processado com segurança</span><span>·</span><span>Seus dados protegidos pela LGPD</span></div><p className="mt-3 text-center text-[11px] leading-relaxed text-muted-foreground">Ao finalizar, você concorda com nossa <Link to="/politicas/$slug" params={{ slug: "trocas" }} className="underline">Política de Trocas e Devoluções</Link> e <Link to="/politicas/$slug" params={{ slug: "privacidade" }} className="underline">Política de Privacidade</Link>.</p></div>;
}

type SummaryProps = { items: CartItem[]; total: number; subtotal: number; shippingPrice: number; shipping: ShippingId | null; coupon: string; setCoupon: (value: string) => void; couponState: "idle" | "success" | "error"; applyCoupon: () => void; couponDiscount: number; pixDiscount: number; payment: Payment; updateItem: (id: string, delta: number) => void; toggleOffer: (offer: (typeof offers)[number]) => void };
function OrderSummary({ items, total, subtotal, shippingPrice, shipping, coupon, setCoupon, couponState, applyCoupon, couponDiscount, pixDiscount, payment, updateItem, toggleOffer }: SummaryProps) {
  return <div><div className="flex items-end justify-between"><h2 className="font-display text-2xl">Seu pedido</h2><span className="text-xs text-muted-foreground">{items.reduce((sum, item) => sum + item.qty, 0)} itens</span></div><div className="mt-4 divide-y divide-checkout-line border-y border-checkout-line sm:mt-5">{items.map((item) => <div key={item.id} className="grid grid-cols-[64px_minmax(0,1fr)_auto] gap-3 py-3 animate-in fade-in duration-300 sm:py-4"><div className="relative"><img src={item.image} alt={item.name} className="aspect-square w-16 rounded-lg bg-checkout-soft object-cover" /><span className="absolute -right-1.5 -top-1.5 grid h-5 min-w-5 place-items-center rounded-full bg-foreground px-1 text-[10px] text-background">{item.qty}</span></div><div className="min-w-0"><p className="text-xs font-semibold leading-snug">{item.name}</p><p className="mt-1 text-[10px] text-muted-foreground">{item.variant}</p>{item.offer && <span className="mt-1.5 inline-block rounded-full bg-success-soft px-2 py-0.5 text-[9px] font-bold text-success">Oferta especial</span>}<div className="mt-2 flex h-8 w-24 items-center justify-between rounded-lg border border-checkout-line"><button type="button" onClick={() => updateItem(item.id, -1)} aria-label={`Diminuir ${item.name}`} className="grid h-full w-8 place-items-center"><Minus className="h-3 w-3" /></button><span className="text-xs">{item.qty}</span><button type="button" onClick={() => updateItem(item.id, 1)} aria-label={`Aumentar ${item.name}`} className="grid h-full w-8 place-items-center"><Plus className="h-3 w-3" /></button></div></div><strong className="text-xs">{brl(item.price * item.qty)}</strong></div>)}</div><OrderBumps items={items} onToggle={toggleOffer} /><div className="mt-4 sm:mt-5"><label htmlFor="coupon" className="mb-2 block text-xs font-semibold">Código de desconto</label><div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2"><Input id="coupon" value={coupon} disabled={couponState === "success"} onChange={(event) => { setCoupon(event.target.value.toUpperCase()); }} placeholder="Ex.: VENUTTI10" className="h-11 rounded-xl border-checkout-line" /><Button type="button" variant="outline" onClick={applyCoupon} disabled={couponState === "success"} className="h-11 rounded-xl">Aplicar</Button></div>{couponState === "success" && <p className="mt-2 flex items-center gap-1 text-xs font-semibold text-success"><CheckCircle2 className="h-3.5 w-3.5" /> Cupom VENUTTI10 aplicado</p>}{couponState === "error" && <p className="mt-2 text-xs text-destructive">Esse cupom não é válido ou já expirou.</p>}</div><dl className="mt-4 space-y-3 border-t border-checkout-line pt-4 text-sm sm:mt-5 sm:pt-5"><div className="flex justify-between"><dt className="text-muted-foreground">Subtotal</dt><dd>{brl(subtotal)}</dd></div><div className="flex justify-between"><dt className="text-muted-foreground">Frete</dt><dd className="max-w-48 text-right">{shipping ? brl(shippingPrice) : "Calculado na etapa de entrega"}</dd></div>{couponDiscount > 0 && <div className="flex justify-between text-success"><dt>Desconto VENUTTI10</dt><dd>- {brl(couponDiscount)}</dd></div>}{pixDiscount > 0 && <div className="flex justify-between text-success"><dt>Desconto no Pix (5%)</dt><dd>- {brl(pixDiscount)}</dd></div>}<div className={`flex items-end justify-between border-t border-checkout-line pt-4 ${payment === "pix" ? "text-success" : ""}`}><dt className="font-semibold">{payment === "pix" ? "Total no Pix" : "Total"}</dt><dd className="font-display text-3xl font-semibold">{brl(total)}</dd></div></dl>{payment === "pix" ? <p className="mt-1 text-right text-xs font-semibold text-success">5% de desconto já aplicado</p> : <p className="mt-2 text-right text-xs text-muted-foreground">ou 12x de {brl(total / 12)} sem juros</p>}</div>;
}
